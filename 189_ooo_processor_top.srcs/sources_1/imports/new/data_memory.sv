...
module data_memory #(
    ...
    logic [31:0] word_addr_now;
    assign word_addr_now = addr[31:2]; // Word aligned address
    ...
    always_ff @(posedge clk) begin
        if (we[0]) mem[word_addr_now][7:0]   <= wdata[7:0];
        if (we[1]) mem[word_addr_now][15:8]  <= wdata[15:8];
        if (we[2]) mem[word_addr_now][23:16] <= wdata[23:16];
        if (we[3]) mem[word_addr_now][31:24] <= wdata[31:24];
    end

    // Cycle 0: register read address
    always_ff @(posedge clk) begin
        if (re) begin
            rd_addr_d0 <= word_addr_now;
        end
        re_d0 <= re;
    end
